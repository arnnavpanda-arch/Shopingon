...
function _hashState(data) {
  const sortedShops = (data.shops || []).slice().sort((a,b) => String(a.id || '').localeCompare(String(b.id || '')));
  const sortedDeliveries = (data.deliveries || []).slice().sort((a,b) => String(a.id || '').localeCompare(String(b.id || '')));
  const sortedOffers = (data.offers || []).slice().sort((a,b) => String(a.id || '').localeCompare(String(b.id || '')));
  const sortedCustomers = (data.customers || []).slice().sort((a,b) => String(a.phone || '').localeCompare(String(b.phone || '')));
...
async function loadStateFromServer(silent = false) {
  try {
    ...
    const response = await fetch('/api/state?t=' + new Date().getTime(), { cache: 'no-store' });
    ...
    if (!silent || newHash !== _lastServerHash) {
      ...
      // Ensure active customer shop catalog updates in real-time
      if (activeCustomerShop) {
        const updatedShop = shops.find(s => s.id === activeCustomerShop.id);
        if (updatedShop) {
          activeCustomerShop = updatedShop;
          if (document.getElementById('cust-shop-catalog').classList.contains('active')) {
            renderShopCatalog();
          }
        }
      }
...
// Silent 3-second background polling for robust synchronization
function startRealTimeSync() {
  ...
  console.log('🔄 Silent background sync started (3s polling)');
}
...
function syncStorage() {
  ...
  .then(data => {
    _lastServerHash = _hashState({ shops, deliveries, offers: activeOffers, customers, settings: {
      totalCouriersPool, baseDeliveryFee, perKmDeliveryFee, dailyCommissions, tieredCommissionRules
    }});
    _showSyncIndicator('ok');
...
window.deleteAgencyOffer = function(id) {
  ...
  fetch('/api/offers/' + encodeURIComponent(id), { method: 'DELETE' })
    .then(res => res.json())
    .catch(err => console.error('Error deleting offer from server:', err));
